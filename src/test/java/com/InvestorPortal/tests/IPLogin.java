...
public class IPLogin extends Baseclass {
	...
	public void invalidUserNameLogin() throws IOException {
		...
		try {
			extenttest = extent.createTest("Verify that user is not able to login  with invalid username.")
					.assignCategory(investorportal_tag + "_" + env);
			node = extenttest.createNode("Report");
	...
	public void invalidPasswordLogin() throws IOException {
		try {
			...
			// Call the enterLoginDetailsAndSubmit function from the utils
	...
	public void validateSignInRedirectionFromForgotPassword() throws IOException {
		try {
			...
			IElementActions.assertTrue_usingXpath(node, driver, LoginPO.signInText,
					"STEP8 : BUG - Sign in page is not displayed", "STEP8 : EXPECTED - Sign in page is displayed");
	...
	public void validateResetPasswordUsingChangePassword() throws IOException {
		try {
			...
			System.out.println(getDataFromExcel(73) + "is new password");
			...
			Utils.clickOnMenu(node, driver, InvestorPortalProfilePO.userprofile);

			// Click on the view personal profile
			Utils.clickOnMenu(node, driver, InvestorPortalProfilePO.viewpersonalprofile);

			Utils.clickOnMenu(node, driver, InvestorPortalProfilePO.changePasswordLink);
			...
			IWaitStrategy.waitForVisiblity(node, driver, InvestorPortalProfilePO.changePasswordLink);
			...
			IElementActions.clickelement_usingXpath(node, driver, InvestorPortalProfilePO.changePasswordLink);
			...
			IWaitStrategy.waitForVisiblity(node, driver, InvestorPortalProfilePO.changePasswordText);
			...
			IElementActions.clickAndInput_usingXpath(node, driver, InvestorPortalProfilePO.oldPassword, password);
			...
			IElementActions.clickAndInput_usingXpath(node, driver, InvestorPortalProfilePO.newPassword, newPassword);
			...
			IElementActions.clickAndInput_usingXpath(node, driver, InvestorPortalProfilePO.confirmPassword,
					newPassword);
			...
			IElementActions.clickelement_usingXpath(node, driver, InvestorPortalProfilePO.saveButton);
			...
			node.log(Status.INFO, "STEP12 : Updated the new password in excel in Current password field");
			System.out.println("STEP12 : Updated the new password in excel in Current password field");
	...
	// TC_IP_Login_012
	@Test(priority = 11)
	public void validateUserLoginByOldPassword() throws IOException, InterruptedException {
		try {
		...
		// Click on confirm password field and input the same new password
		IElementActions.clickAndInput_usingXpath(node, driver, InvestorPortalProfilePO.confirmPassword, newPassword);
		node.log(Status.INFO, "STEP10 : Confirm password entered by user on the change password");
		System.out.println("STEP10 : Confirm password entered by user on the change password");
